...
k = config.k
grad_accum = getattr(config, 'grad_accum', 30)  # default 30 for backward compat
...
def val_evaluation():
    ...
    print('Val F1: {:.4f}, Precision: {:.4f}, Recall: {:.4f}'.format(
        np.nanmean(pp_exact_f1), np.nanmean(pp_exact_p), np.nanmean(pp_exact_r)))
